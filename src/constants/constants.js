export const projects = [
  {
    title: 'Watchme',
    description: "A Netflix-clone created using React and Firebase. React Compound Components design pattern is utilized combined with styled components. React hooks and React context API were used to produce reusable, easily readable, react functional components. Cloud Firestore is used as the database and Firebase authentication is used for user signing up, signing in and monitoring user auth state.",
    image: '/images/1.jpg',
    tags: ['React', 'Firebase', 'CSS', 'gh-pages'],
    visit: 'https://sherifessawy.github.io/watchme/',
    source: 'https://github.com/sherifessawy/watchme',
    id: 0,
  },
  {
    title: 'E-Commerce',
    description:"sample E-commerce web app with some basic functionality like: logging in/out, favoriting items, adding items to cart and placing orders",
    image: '/images/2.jpg',
    tags: ['React', 'CSS', 'HTML', 'gh-pages'],
    visit: 'https://sherifessawy.github.io/e-commerce/',
    source: 'https://github.com/sherifessawy/e-commerce',
    id: 1,
  },
  {
    title: 'Simulator',
    description: "A Race Cars Track Simulation Tool developed using python. It helped in the design and development of racing Formula Student Race cars. The Code and the output was concluded in a technical paper presented at WCX, SAE World Congress Experience conference",
    image: '/images/3.jpg',
    tags: ['Python', 'Jupyter Notebook', 'SAE'],
    visit: 'https://www.sae.org/publications/technical-papers/content/2019-01-1128/',
    source: 'https://github.com/sherifessawy/Track-Simulation-Tool',
    id: 2,
  },
  {
    title: 'Games',
    description: "blank",
    image: '/images/4.jpg',
    tags: ['blank', 'blank', 'blank'],
    visit: '',
    source: '',
    id: 3,
  },
];

export const TimeLineData = [
  { year: 2018, text: 'Started my journey', },
  { year: 2019, text: 'Presented my first software project at SAE international conference', },
  { year: 2021, text: 'Worked as a forntend webdev freelancer'}
];